...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Get all existing Jira tasks for this user (only active ones for reassignment check)
    ...
    for (const [issueKey, task] of existingByKey) {
      if (task.jira_sync_status === 'active' && !currentJiraKeys.has(issueKey)) {
        ...
        if (!unassignError) {
          ...
          errors.push(`Failed to mark ${issueKey} as unassigned: ${unassignError.message}`)
        }
      }
    }
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: `Sync complete: ${created} created, ${updated} updated${markedUnassigned > 0 ? `, ${markedUnassigned} removed (reassigned)` : ''}`,
...
function mapTshirtSize(storyPoints: any): { energy_level: string; time_estimate: number; jira_tshirt_size: string | null } {
  // Story points / T-shirt sizes come in various formats
 ...
 * Sprint field (customfield_10020) is an array of sprints
 * We want the active one, or the most recent future one
 */
function extractActiveSprint(sprints: any[]): { id: string; name: string; state: string } | null {
  ...
  // Then look for future sprint (scheduled but not started)
  ...
  // Fall back to most recent (last in array, usually most recent closed)
